    						...
    						<fieldset>
    							...
    							<div class="control-group-linha">
    								...
    								<div class="controls padding-visualiza">
                                        <input class="datepicker" type="text" maxlength="10" name="data" id="data" value="<?=date("d/m/Y", strtotime($ped[6]))?>"<?=$acao == "visualizar" ? " readonly" : ""?>/>
    								</div>
    							</div>
														...
														<td style="background:#fff">
															<div class="tabbable">
																...
																<div class="tab-content">
                                                                    ...
                                                                    <?php
                                                                        $parcela = mysql_query("SELECT * FROM `parcelas` WHERE `parcelas_id` = $id");
                                                                        $cont = 1;
                                                                        $total = mysql_num_rows($parcela);
                                                                        if(mysql_num_rows($parcela) > 1):
                                                                    ?>
                                                                        <div id="div_parcelamento">
                                                                                <strong>Parcelada em <?=$total?> vezes </strong><br/><br/>
                                                                    <?php while($par = mysql_fetch_row($parcela)): ?>
                                                                            <?=$cont?>/<?=$total?>&nbsp;&nbsp;&nbsp;<input type="text" class="span1" id="parcela_<?=$cont?>" name="parcela_<?=$cont?>" value="<?=number_format($par[2], 2, ",", ".")?>" style="width: 100px;"<?=$acao == "visualizar" ? " readonly" : ""?>/>&nbsp;&nbsp;&nbsp;<input type="text" id="data_<?=$cont?>" name="data_<?=$cont?>" class="datepicker hasDatepick hasDatepicker" value="" <?=$acao == "visualizar" ? " readonly" : ""?>/><br/>
                                                                            <script>
                                                                                $('#parcela_<?=$cont?>').maskMoney({showSymbol:true, symbol:'', decimal:',', thousands:'.'})
                                                                            </script><br/>	
																			<!--<img src="application/img/ajax-loader-progress-bar.gif">-->
																		</div>
                                                                    <?php
                                                                            endwhile;
                                                                        else:
                                                                    ?>
                                                                        <div class="padding">
																			<div id="div_parcelamento">
																				<strong>Definir a data do Pagamento &agrave; vista</strong>&nbsp;<input type='text' id='data_1' name='data_1' class='span2 datepicker' value=''<?=$acao == "visualizar" ? " readonly" : ""?>/>
																				<!--<img src="application/img/ajax-loader-progress-bar.gif">-->
																			</div>
																		</div>
                                                                    <?php
																	...
																	<div class="tab-pane fade" id="tab2">
                                                                    	<div class="padding">	
                                                                            ...
                                                                            <?php
                                                                                    ...
                                                                                    $comissao = mysql_query("SELECT `vendedor_comissao` FROM `vendedor` WHERE `pessoa_id` = " . $ped[10]);
                                                                                    $com = mysql_fetch_row($comissao);
                                                                                    $vl_comisssao = (float)$parc[8] / (float)$com;    
                                                                            ?>
																				<strong>Comiss&otilde;es:</strong><br/>
                                                                                <strong><i class='icon-chevron-right'></i> R$ <?=number_format($vl_comisssao, 2, ",", ".")?> </strong><strong> no dia <input type='text' name='data_comissao' id='data_comissao' class='span2 datepicker' value=''<?=$acao == "visualizar" ? " readonly" : ""?>/></strong><br/> 
																			<?php endif; ?>
                                                                            </div>
																		</div>
																	</div>
