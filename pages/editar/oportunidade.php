<?php
    ...
    if($_GET) {
        ...
        if(!empty($id)) {
            $query = mysql_query("SELECT `o`.`oportunidade_id`, `o`.`oportunidade_nome`, `u`.`usuario_nome`, `o`.`usuario_id`, `o`.`oportunidade_data`, `v`.`contato_nome`, `o`.`pessoa_vendedor`, `o`.`oportunidade_descricao`, `o`.`oportunidade_neg_vl`, `o`.`oportunidade_periodo`, `o`.`oportunidade_vl_periodo`, `ca`.`conta_razao_social`, `o`.`pessoa_conta`, `co`.`contato_nome`, `o`.`pessoa_contato`, `o`.`oportunidade_dt_fec`, `o`.`status_oportunidade_id`, `o`.`oportunidade_chance_suc`, IFNULL(`ct`.`conta_razao_social`, `ctt`.`contato_nome`), `o`.`pessoa_concorrente`, IF(`o`.`pessoa_concorrente` <> NULL, IF(`ct`.`conta_id` <> NULL, 'conta', 'contato'), ''), `o`.`tipo_parcela_id`, `o`.`oportunidade_pedido`, `o`.`oportunidade_repete_dt` FROM `oportunidade` `o` JOIN `usuario` `u` ON `o`.`usuario_id` = `u`.`usuario_id` LEFT JOIN `contato` `v` ON `o`.`pessoa_vendedor` = `v`.`pessoa_id` LEFT JOIN `conta` `ca` ON `o`.`pessoa_conta` = `ca`.`pessoa_id` JOIN `contato` `co` ON `o`.`pessoa_contato` = `co`.`pessoa_id` LEFT JOIN `pessoa` `p` ON `o`.`pessoa_concorrente` = `p`.`pessoa_id` LEFT JOIN `conta` `ct` ON `p`.`pessoa_id` = `ct`.`pessoa_id` LEFT JOIN `contato` `ctt` ON `p`.`pessoa_id` = `ctt`.`pessoa_id` WHERE `o`.`oportunidade_id` = $id");
            if(mysql_num_rows($query) > 0) {
                ...
                while($prod = mysql_fetch_row($query)) {
                    mysql_query("INSERT INTO `itens_temporario` VALUES ({$_COOKIE['id']}, {$prod[0]}, {$prod[1]}, {$prod[2]})");
                }
            } else {
							...
							<form>
								<fieldset>
									...
									<div class="padding well">	
										...
										<?php if(!empty($oport[5]) && $qtd_parc > 0): ?>
                                        <input style="width:100px;" class="datepicker" type="text" maxlength="10" name="data_parcelas" id="data_parcelas" onkeydown="Mascara(this,Data);" onkeypress="Mascara(this,Data);" onkeyup="Mascara(this,Data);" value="<?=date("d/m/Y", strtotime($ped_parc[0][1]))?>"/>
                                        <?php else: ?>
                                        <input style="width:100px;" class="datepicker" type="text" maxlength="10" name="data_parcelas" id="data_parcelas" onkeydown="Mascara(this,Data);" onkeypress="Mascara(this,Data);" onkeyup="Mascara(this,Data);" value=""/>
                                        <?php endif; ?>
									...
									<div class="padding well" id="div_comissao" style="display:none;">	
										<div class="control-group">
                                            ...
                                            <?php
                                                if(!empty($oport[6])) {
                                                    $com = mysql_query("SELECT `comissao_data` FROM `comissao` where `oportunidade_id` = $id");
                                                    $dt_com = mysql_fetch_row($com);
                                                }
                                            ?>
											<input style="width:100px;" class="datepicker" type="text" maxlength="10" name="data_comissoes" id="data_comissoes" class="input_menor" onkeydown="Mascara(this,Data);" onkeypress="Mascara(this,Data);" onkeyup="Mascara(this,Data);" value="<?=!empty($oport[6]) ? date("d/m/Y", strtotime($dt_com[0])) : "" ?>"/>
										</div>
							...
							<div class="tab-content">
								<div class="tab-pane fade in active" id="tab1">
									<form class="form-horizontal" action="http://<?=$_SERVER['HTTP_HOST']?>/home/oportunidades/valida/inserir" id="form_opt" method="post" enctype="multipart/form-data" name="form_opt">
										<fieldset>
											<input type="hidden" name="gerar_pedido" id="gerar_pedido" value="<?=!empty($oport[22]) ? "1" : ""?>" />
                                            <input type="hidden" name="cod_opt" id="cod_opt" value="<?=$oport[0]?>"/>
											<!-- campos pedido -->
											<input type="hidden" name="hpedido_acao" id="hpedido_acao" value="<?=!empty($oport[22]) ? "gerar" : ""?>" />
											<input type="hidden" name="hrepetir_data_parcelas" id="hrepetir_data_parcelas" value="<?=$oport[23]?>" />
											<input type="hidden" name="hdata_comissoes_pedido" id="hdata_comissoes_pedido" value="<?=!empty($oport[6]) ? date("d/m/Y", strtotime($dt_com[0])) : "" ?>" />
											...
											<input type="hidden" name="hdata_parcelas_pedido" id="hdata_parcelas_pedido" value="<?=date("d/m/Y", strtotime($ped_parc[0][1]))?>" />
											...
											<div class="control-group">
												<label class="control-label">* Data:</label>
												<div class="controls">
													<input class="datepicker" type="text" maxlength="10" name="data" id="data" class="input_menor" value="<?=date("d/m/Y", strtotime($oport[4]))?>"/>
												</div>
											</div>
											<div class="control-group">
												<label class="control-label">* Oportunidade:</label>
												<div class="controls">
													<input type="text" name="nome" id="nome" class="input_full" value="<?=$oport[1]?>"/>
												</div>
											</div>
											<div id="div_vendedor" class="control-group">
												<label class="control-label">Vendedor:</label>
												<div class="controls">
													<input class="input_full ui-autocomplete-input" type="text" id="vendedor" name="vendedor" placeholder="Buscar" value="<?=$oport[5]?>" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true"/>
													<input type="hidden" name="codvendedor" id="codvendedor" value="<?=$oport[6]?>"/>
												</div>
											</div>
											<div class="control-group">
												<label class="control-label">Descri&ccedil;&atilde;o:</label>
												<div class="controls">
													<textarea class="input_full" name="descricao" id="descricao"><?=$oport[7]?></textarea>
												</div>
											</div>
											<div>  
											...
											<div class="control-group">
												...
												<div class="controls  ">
													<input type="text" placeholder="R$" class="span1" size="8px"  name="servico" id="servico" value="<?=number_format($oport[8], 2, ",", ".")?>" onblur="CalculaTotal();CriarInputs($('#qtd_parcelas').val());"/>
													...
													?>
															<option value="<?=$parc[1]?>"<?=$oport[8] == $parc[0]? " selected='true'" : ""?>><?=$parc[2]?></option>
													<?php endwhile; ?>
													</select>
													<span id="div_parcelas" style='display:<?=$oport[21] > 1 ? "inline;" : "none;"?>'>
														<b class="">Durante</b>
														<input class="span1" type="text" name="parcela_servico" id="parcela_servico"  value="<?=$oport[9]?>" onkeyup="Mascara(this,Integer);" onkeypress="Mascara(this,Integer);" onkeydown="Mascara(this,Integer);"  onblur="CalculaTotal();CriarInputs($('#qtd_parcelas').val())"/>
														<span id="tipo_parcelas" style='display:<?=$oport[8] != 1 ? "inline;" : "none;"?>'>
															<strong><?=$oport[8] != 1 ? ucfirst($oport[10])."s" : ""?></strong>
														</span>
														<strong id='strong_total' style='display:<?=$oport[8] != 1 ? "inline;" : "none;"?>'></strong>
													</span>
												</div>
											</div>
											<div>  
												...
												<div class="control-group">
													<div class="controls well" id="div_negociacao">
														<label class="radio inline">
															<input type="radio" name="negociacao" value="vista" id="negociacao_vista" onclick="CheckNegociacao(this.value);" <?=$qtd_parc == 1 ? "checked='true'" : ""?>/>&Agrave; vista
														</label>
														<br/>
														<label class="radio inline">
															<input type="radio" name="negociacao" value="parcelada" id="negociacao_parcelada" onclick="CheckNegociacao(this.value);" <?=$qtd_parc > 1 ? "checked='true'" : ""?>/>Parcelada em
														</label>
                                                        
														<label class="radio inline" id="div_qtd_parcelas" style='<?=$qtd_parc > 1 ? "" : "display:none;"?>'>
															<input type="text" value="<?=$qtd_parc?>" id="qtd_parcelas" name="qtd_parcelas" size="8px" class="span1" placeholder="X" align="center" onblur="CriarInputs(this.value);"/>&nbsp;&nbsp;Vezes
                                                        ...
                                                        <?php 
                                                            $i = 1;
                                                            if($qtd_parc > 1): 
                                                                foreach($ped_parc AS $valor): ?>
															<br /><?=$i?>/<?=$qtd_parc?>&nbsp;&nbsp;&nbsp;
															<input type="text" class="span1" id="parcela_<?=$i?>" name="parcela_<?=$i?>" value="<?=$valor[0]?>"/>&nbsp;&nbsp;&nbsp;<br />
															<script>
											...
											<div class="control-group">
												...
												<div class="controls">
													<div class="pr_5">
														<label class="radio inline">
															<input type="radio" onclick="ConfigProspect(this.value);" name="prospect" value="conta" id="prospect_conta" <?=!empty($oport[11]) ? "checked='true'" : ""?>/>Conta
														</label>
														<label class="radio inline">
															<input type="radio" onclick="ConfigProspect(this.value);" name="prospect" value="contato" id="prospect_contato" <?=empty($oport[11]) ? "checked='true'" : ""?>/>Contato
														</label>
														&nbsp;
											...
											<div id="div_conta" style='<?=!empty($oport[11]) ? "" : "display:none;"?>'  class="control-group">
												<label class="control-label">Conta:</label>
												<div class="controls">
													<input class="input_full" type="text" id="conta" placeholder="Buscar" value="<?=$oport[11]?>"/>
													<input type="hidden" name="nomeconta" id="nomeconta" value="<?=$oport[11]?>"/>
													<input type="hidden" name="codconta" id="codconta" value="<?=$oport[12]?>"/>
												</div>
											</div>
											<div id="div_contato"  style='<?=!empty($oport[13]) ? "" : "display:none;"?>' class="control-group">
												<label class="control-label">Contato:</label>
												<div class="controls">
													<input class="input_full" type="text" id="contato" placeholder="Buscar" value="<?=$oport[13]?>"/>
													<input type="hidden" name="nomecontato" id="nomecontato" value="<?=$oport[13]?>"/>
													<input type="hidden" name="codcontato" id="codcontato" value="<?=$oport[14]?>"/>
												</div>
											</div>
											<div class="control-group">
												<label class="control-label">* Fechamento prov&aacute;vel:</label>
												<div class="controls">
													<input type="text" class="span2 datepicker" placeholder="" maxlength="10" onkeydown="Mascara(this,Data);" onkeypress="Mascara(this,Data);" onkeyup="Mascara(this,Data);" name="fechamento" id="fechamento" class="input_menor" value="<?=date("d/m/Y", strtotime($oport[15]))?>"/>
												</div>
											</div>
											<div class="control-group">
												...
												<div class="controls">
													<div class="btn-group flt_lft">
														<label onclick="Situacao(this.id);" id="andamento" class='btn dropdown-toggle<?=$oport[16] == 2 ? " btn-warning" : ""?>' href="#">Andamento</label>
													</div>
													<div class="btn-group flt_lft">
														<label onclick="Situacao(this.id);" id="ganhamos" class='btn dropdown-toggle<?=$oport[16] == 1 ? " btn-success" : ""?>' href="#">Ganhamos</label>
													</div>
													<div class="btn-group flt_lft">
														<label onclick="Situacao(this.id);" id="perdemos" class='btn dropdown-toggle'<?=$oport[16] == 0 ? " btn-danger" : ""?> href="#">Perdemos</label>
													</div>&nbsp;
													...
													<input type="hidden" id="situacao" name="situacao" value="<?=$oport[16]?>"/>
												</div>
											</div>
											<div class="control-group">
												<label class="control-label">Chance de sucesso:</label>
												<div class="controls">
													<input class="alinhaDireita" type="text" size="2px" maxlength="100" name="chance" id="chance" class="input_menor" onkeydown="Mascara(this,Integer);" onkeypress="Mascara(this,Integer);" onkeyup="Mascara(this,Integer);" value="<?=$oport[17]?>"/>&nbsp;<b>%</b>
												</div>
											</div>
											<div class="control-group">
												...
												<div class="controls">
													...
													<input type="hidden" name="nomecon" id="nomecon" value="<?=$oport[18]?>"/>
													<input type="hidden" name="codcon" id="codcon" value="<?=$oport[19]?>"/>
													<input type="hidden" name="tipocon" id="tipocon" value="<?=$oport[20]?>"/>
												</div>
											</div>
