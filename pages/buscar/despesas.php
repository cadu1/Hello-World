...
<div class="row-fluid" style="text-align:center;">
	<table class="da-table">
		...
		<tbody>
            ...
            $query = mysql_query("SELECT `l`.`lancamento_id`, `l`.`lancamento_vencimento`, `ca`.`categoria_nome`, `l`.`lancamento_descricao`, `l`.`pessoa_cliente`, `l`.`lancamento_valor`, `c`.`caixa_nome` FROM `lancamento` `l` LEFT JOIN `caixa` `c` ON `l`.`caixa_id` = `c`.`caixa_id` LEFT JOIN `conta_corrente` `cc` ON `c`.`caixa_id` = `cc`.`caixa_id` JOIN `categoria` `ca` ON `l`.`categoria_id` = `ca`.`categoria_id` WHERE `ca`.`categoria_tipo` IS FALSE AND `l`.`lancamento_vencimento` $where");
